...
const INITIAL_FAN_QUESTIONS = [
  {
    id: "qa-1",
    eventId: "solstice-2026",
    eventTitle: "Solstice Music Festival",
    userName: "Sophia Sterling",
    userEmail: "sophia@luxe.design",
    questionText: "Will there be any surprise guest sets during the midnight transition?",
    timestamp: "04:30 PM",
    status: "answered",
    reply: "We have two unannounced guest sets lined up! Keep your eyes on the laser arches at midnight."
  },
  {
    id: "qa-2",
    eventId: "solstice-2026",
    eventTitle: "Solstice Music Festival",
    userName: "Alex Mercer",
    userEmail: "alex@luxe.design",
    questionText: "Is the backstage glass lounge heated? It gets chilly in San Francisco in June.",
    timestamp: "05:15 PM",
    status: "pending",
    reply: ""
  }
...
export const mockDb = {
  ...
  init: () => {
    ...
    if (!localStorage.getItem("luxe_fan_questions")) {
      localStorage.setItem("luxe_fan_questions", JSON.stringify(INITIAL_FAN_QUESTIONS));
    }
  },
  ...
  // Get all Q&A questions, optionally filtered by eventId
  getFanQuestions: (eventId = null) => {
    if (!isClient) return INITIAL_FAN_QUESTIONS;
    mockDb.init();
    const questions = JSON.parse(localStorage.getItem("luxe_fan_questions")) || [];
    if (eventId) {
  ...
  submitFanQuestion: (questionData) => {
    if (!isClient) return null;
    mockDb.init();
    const questions = JSON.parse(localStorage.getItem("luxe_fan_questions")) || [];
    const newQuestion = {
      id: "qa-" + Math.floor(Math.random() * 900000 + 100000),
      eventId: questionData.eventId,
      eventTitle: questionData.eventTitle,
    ...
    localStorage.setItem("luxe_fan_questions", JSON.stringify(questions));
    return newQuestion;
  },

  // Answer a question
  answerFanQuestion: (questionId, replyText) => {
    if (!isClient) return false;
    mockDb.init();
    const questions = JSON.parse(localStorage.getItem("luxe_fan_questions")) || [];
    const idx = questions.findIndex(q => q.id === questionId);
    if (idx === -1) return false;
    questions[idx].reply = replyText;
  ...
  // Delete a question
  deleteFanQuestion: (questionId) => {
    if (!isClient) return false;
    mockDb.init();
    const questions = JSON.parse(localStorage.getItem("luxe_fan_questions")) || [];
    const filtered = questions.filter(q => q.id !== questionId);
    localStorage.setItem("luxe_fan_questions", JSON.stringify(filtered));
    return true;
  }
};
