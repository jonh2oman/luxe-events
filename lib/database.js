...
export const database = {
  ...
  // --- Resale Exchange ---
  listTicketForResale: async (bookingId, price) => {
    return mockDb.listTicketForResale(bookingId, price);
  },

  delistTicketForResale: async (bookingId) => {
    return mockDb.delistTicketForResale(bookingId);
  },

  getResaleListings: async () => {
    return mockDb.getResaleListings();
  },

  buyResaleListing: async (listingId, buyerName, buyerEmail) => {
    return mockDb.buyResaleListing(listingId, buyerName, buyerEmail);
  },

  // --- Fan Mail & Q&A Board ---
  getFanQuestions: async (eventId = null) => {
    return mockDb.getFanQuestions(eventId);
  },

  submitFanQuestion: async (questionData) => {
    return mockDb.submitFanQuestion(questionData);
  },

  answerFanQuestion: async (questionId, replyText) => {
    return mockDb.answerFanQuestion(questionId, replyText);
  },

  deleteFanQuestion: async (questionId) => {
    return mockDb.deleteFanQuestion(questionId);
  }
};
